...
const { data, status,clear} = await useCategoryPlaces( query.id );
clear()
</script>
    ...
    <div v-if="data">
      ...
      <VRow class="mt-2">
        ...
        >
         ...
         >
           <VImg  @click="$router.push({path:'/placeServices',query: { id:placeService.id }})" width="130"  rounded="lg" :src="placeService.images[0]?.image?.url" />
